...
size_t N = 8;
double dt = 0.1; //match the delay 100ms
...
class FG_eval {
 public:
  ...
  FG_eval(Eigen::VectorXd coeffs, vector<double> actuations) {
    this->coeffs = coeffs;
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    // this code segment is taken from the MPC quiz with corrections of the cte definition
    // TODO: implement MPC
    ...
    cout <<  actuations[0] << ' ' <<  actuations[1] << endl;
    for (int t = 1; t < N; t++) {
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  // steering angles bound to -deg2rad(25)  & deg2rad(25)
  for (int i = delta_start; i < a_start; i++) {
    vars_lowerbound[i] = -0.436332;
    vars_upperbound[i] = 0.436332;
  }

  // accelerations bound to -1 & 1
  ...
  //set initial states/constriants
  ...
  ret.push_back(N);
  for (int i = x_start + 1; i <= y_start; i++) ret.push_back(solution.x[i]);
  for (int i = y_start + 1; i <= psi_start; i++) ret.push_back(solution.x[i]);
  return ret;
}
